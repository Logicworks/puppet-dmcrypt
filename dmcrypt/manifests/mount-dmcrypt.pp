  ...
  # <mount_point><dmcrypt_password>[<dmcrypt_name>]
  #
...
# arguments
#
define decrypt_and_mount ($input_device, $input_mount_point, $input_dmcrypt_name) {
    ...
    exec { "decrypt_dmcrypt_device":
        ...
        command => "/sbin/cryptsetup luksOpen /dev/${device} ${input_dmcrypt_name}",
    }
    exec { "mount_dmcrypt_device":
        require => Exec['decrypt_dmcrypt_device'],
        unless => "/bin/mount |grep '/dev/mapper/${input_dmcrypt_name}",
        command => "/bin/mount /dev/mapper/${input_dmcrypt_name} ${mount_point}",
    }
}
